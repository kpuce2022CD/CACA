...
app.get('/About',(req, res)=>{
  fs.readFile('views/About.ejs', 'utf8', function (err, data) {  // about.ejs 파일을 읽어옴. (파일 경로/파일명 , 파일에 들어갈 내용)
    connection.query("SELECT about FROM user where user_email = \'" + email + "\'", function (err, rows) {  // user1 자리에 있는건 유동적으로 바뀜.
      ...
      // user1 자리에는 소켓 통신으로 입력받은 값이 들어감.
      if (err) {
        res.send(err)  // 에러 출력
      } else {
...
app.get('/aboutEdit/:id', function (req, res) {  // :id 는 파라미터 정의할때 사용하는 경로
  fs.readFile('views/aboutEdit.ejs', 'utf8', function (err, data) {   // view 내부의 aboutedit 읽어옴.
    connection.query("update about SET user_id = 'user1'", [req.params.id], function (err, result) {  // 파라미터 id 요청
...
app.post('/aboutEdit/:id', function (req, res) {  // 매 아이디를 부여해야 한다.
  const body = req.body

  // user_id가 user1 인 곳의 DB 내용을 최신화
  connection.query("update about SET user_email = \'" + email + "\'",[
    // body는 aboutEdit 내의 내용임.
    body.about, req.params.id  // aboutEdit의 body내 data.about에 해당하는 요소를 업데이트
  ], function () {
    res.redirect('/About')  // 메인페이지로 돌아감.
  })
})
})
