  ...
  `;
    ...
    describe('Document Symbols Tests (Non Hierarchical)', function () {
        ...
        it('Document symbols with a limit', () => {
            ...
            assert.equal(symbols.length, 1);
            assert.deepEqual(symbols[0], verifyError_1.createExpectedSymbolInformation('a', vscode_languageserver_types_1.SymbolKind.Array, '', testHelper_1.TEST_URI, 1, 4, 1, 16));
            assertLimitWarning();
        });
    });
    describe('Document Symbols Tests (Hierarchical)', function () {
        ...
        it('Simple document symbols', () => {
            ...
            const symbols = parseHierarchicalSetup(content);
            assert.equal(symbols.length, 1);
            assert.deepEqual(symbols[0], verifyError_1.createExpectedDocumentSymbol('cwd', 15, 0, 0, 0, 9, 0, 0, 0, 3, [], 'test'));
        });
        it('Document Symbols with number', () => {
            const content = 'node1: 10000';
            const symbols = parseHierarchicalSetup(content);
            assert.equal(symbols.length, 1);
            assert.deepEqual(symbols[0], verifyError_1.createExpectedDocumentSymbol('node1', 16, 0, 0, 0, 12, 0, 0, 0, 5, [], '10000'));
        });
        it('Document Symbols with boolean', () => {
            const content = 'node1: False';
            const symbols = parseHierarchicalSetup(content);
            assert.equal(symbols.length, 1);
            assert.deepEqual(symbols[0], verifyError_1.createExpectedDocumentSymbol('node1', 17, 0, 0, 0, 12, 0, 0, 0, 5, [], 'false'));
        });
        it('Document Symbols with object', () => {
            const content = 'scripts:\n  node1: test\n  node2: test';
            const symbols = parseHierarchicalSetup(content);
            assert.equal(symbols.length, 1);
            const child1 = verifyError_1.createExpectedDocumentSymbol('node1', vscode_languageserver_types_1.SymbolKind.String, 1, 2, 1, 13, 1, 2, 1, 7, [], 'test');
            const child2 = verifyError_1.createExpectedDocumentSymbol('node2', vscode_languageserver_types_1.SymbolKind.String, 2, 2, 2, 13, 2, 2, 2, 7, [], 'test');
        ...
        it('Document Symbols with null', () => {
            ...
            const symbols = parseHierarchicalSetup(content);
            assert.equal(symbols.length, 1);
            assert.deepEqual(symbols[0], verifyError_1.createExpectedDocumentSymbol('apiVersion', vscode_languageserver_types_1.SymbolKind.Variable, 0, 0, 0, 16, 0, 0, 0, 10, [], 'null'));
        });
        it('Document Symbols with array of strings', () => {
            const content = 'items:\n  - test\n  - test';
            const symbols = parseHierarchicalSetup(content);
            assert.equal(symbols.length, 1);
            const child1 = verifyError_1.createExpectedDocumentSymbolNoDetail('0', vscode_languageserver_types_1.SymbolKind.String, 1, 4, 1, 8, 1, 4, 1, 8);
            const child2 = verifyError_1.createExpectedDocumentSymbolNoDetail('1', vscode_languageserver_types_1.SymbolKind.String, 2, 4, 2, 8, 2, 4, 2, 8);
            const children = [child1, child2];
        ...
        it('Document Symbols with array', () => {
            const content = 'authors:\n  - name: Josh\n  - email: jp';
            const symbols = parseHierarchicalSetup(content);
            const object1 = verifyError_1.createExpectedDocumentSymbol('name', vscode_languageserver_types_1.SymbolKind.String, 1, 4, 1, 14, 1, 4, 1, 8, [], 'Josh');
            const arrayChild1 = verifyError_1.createExpectedDocumentSymbolNoDetail('0', vscode_languageserver_types_1.SymbolKind.Module, 1, 4, 1, 14, 1, 4, 1, 14, [object1]);
            const object2 = verifyError_1.createExpectedDocumentSymbol('email', vscode_languageserver_types_1.SymbolKind.String, 2, 4, 2, 13, 2, 4, 2, 9, [], 'jp');
            const arrayChild2 = verifyError_1.createExpectedDocumentSymbolNoDetail('1', vscode_languageserver_types_1.SymbolKind.Module, 2, 4, 2, 13, 2, 4, 2, 13, [object2]);
        ...
        it('Document Symbols with object and array', () => {
            ...
            const children = [child1, child2];
            assert.deepEqual(symbols[0], verifyError_1.createExpectedDocumentSymbol('scripts', vscode_languageserver_types_1.SymbolKind.Module, 0, 0, 2, 13, 0, 0, 0, 7, children));
            const object1 = verifyError_1.createExpectedDocumentSymbol('name', vscode_languageserver_types_1.SymbolKind.String, 4, 4, 4, 14, 4, 4, 4, 8, [], 'Josh');
            const arrayChild1 = verifyError_1.createExpectedDocumentSymbolNoDetail('0', vscode_languageserver_types_1.SymbolKind.Module, 4, 4, 4, 14, 4, 4, 4, 14, [object1]);
            const object2 = verifyError_1.createExpectedDocumentSymbol('email', vscode_languageserver_types_1.SymbolKind.String, 5, 4, 5, 13, 5, 4, 5, 9, [], 'jp');
            const arrayChild2 = verifyError_1.createExpectedDocumentSymbolNoDetail('1', vscode_languageserver_types_1.SymbolKind.Module, 5, 4, 5, 13, 5, 4, 5, 13, [object2]);
        ...
        it('Document Symbols with multi documents', () => {
            ...
            const symbols = parseHierarchicalSetup(content);
            assert.equal(symbols.length, 2);
            assert.deepEqual(symbols[0], verifyError_1.createExpectedDocumentSymbol('analytics', vscode_languageserver_types_1.SymbolKind.Boolean, 1, 0, 1, 15, 1, 0, 1, 9, [], 'true'));
            assert.deepEqual(symbols[1], verifyError_1.createExpectedDocumentSymbol('json', vscode_languageserver_types_1.SymbolKind.String, 4, 0, 4, 10, 4, 0, 4, 4, [], 'test'));
        });
        it('Document Symbols with complex mapping and aliases', () => {
            ...
            const symbols = parseHierarchicalSetup(content);
            assert.equal(symbols.length, 3);
            assert.deepEqual(symbols[0], verifyError_1.createExpectedDocumentSymbol('version', vscode_languageserver_types_1.SymbolKind.String, 1, 12, 1, 26, 1, 12, 1, 19, [], '0.0.1'));
            const element = verifyError_1.createExpectedDocumentSymbol('element', vscode_languageserver_types_1.SymbolKind.String, 5, 16, 5, 28, 5, 16, 5, 23, [], 'div');
            ...
            const height = verifyError_1.createExpectedDocumentSymbol('height', vscode_languageserver_types_1.SymbolKind.Number, 10, 18, 10, 28, 10, 18, 10, 24, [], '41');
            ...
            const root2 = verifyError_1.createExpectedDocumentSymbol('root', vscode_languageserver_types_1.SymbolKind.Module, 7, 16, 10, 28, 7, 16, 7, 21, [style]);
        ...
        it('Document symbols with a limit', () => {
            ...
            const el = verifyError_1.createExpectedDocumentSymbolNoDetail('0', vscode_languageserver_types_1.SymbolKind.Number, 1, 8, 1, 9, 1, 8, 1, 9, []);
            const root = verifyError_1.createExpectedDocumentSymbol('a', vscode_languageserver_types_1.SymbolKind.Array, 1, 4, 1, 16, 1, 4, 1, 5, [el]);
