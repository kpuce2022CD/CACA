...
export class SchemaCollectorImpl {
...
export function validate(node, document, schema, originalSchema, validationResult, matchingSchemas, options
// eslint-disable-next-line @typescript-eslint/no-explicit-any
) {
        ...
        if (Array.isArray(schema.required)) {
            for (const propertyName of schema.required) {
                if (!seenKeys[propertyName]) {
                    const parent = getParent(document, node);
                    const keyNode = parent && isPair(parent) && parent.key;
                    const [offset, length] = toOffsetLength(node.range);
            ...
            for (const propertyName of Object.keys(schema.properties)) {
                ...
                if (child) {
                    if (isBoolean(propertySchema)) {
                        if (!propertySchema) {
                            const propertyNode = getParent(document, child);
                            const [offset, length] = toOffsetLength(propertyNode.key.range);
                            validationResult.problems.push({
                                location: {
                                    offset,
                                    length,
            ...
            if (unprocessedProperties.length > 0) {
                for (const propertyName of unprocessedProperties) {
                    ...
                    if (child) {
                        ...
                        if (!isPair(child)) {
                            propertyNode = getParent(document, child);
