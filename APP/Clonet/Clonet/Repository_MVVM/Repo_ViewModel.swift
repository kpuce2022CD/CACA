...
//    }
    ...
    func DeleteRequest(user_id: String, x_pixel: String, y_pixel: String, request_context: String) {
        Network.shared.apollo.perform(mutation: DeleteRequestContextMutation(deleteRequestContextUserId2: user_id, xPixel: x_pixel, yPixel: y_pixel, requestContext: request_context)) { result in
            switch result {
            case .success(let graphQLResult):
